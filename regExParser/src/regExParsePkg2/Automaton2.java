...
class Buffer implements Comparable<Buffer> {
...
public class Automaton2 {
	...
	public LinkedList<String> sortedAlph = new LinkedList<String>(); // alphabet for the automaton
		...
		if (s.startsWith("CONCAT")) {
			...
			// add link from final states of aut1 to new buffer created.
			{
				...
				while (it.hasNext()) {
					...
					String action = aut1.finalBuf.num;
					aut.addStateBufferLink(aut.getState(s1.num), newBuf, action);
					action = aut.finalBuf.num + " * " + aut1.finalBuf.num;
					aut.addStateBufferLink(aut.getState(s1.num), newBuf, action);
				}
			}
			...
			// switch final and non-final states
			{
				LinkedHashSet<State> aut1FinalCopy = new LinkedHashSet<State>();
				aut1FinalCopy.addAll(aut1.finalSt);
				Iterator<State> it1 = aut1.states.iterator();
				aut1.finalSt.clear();
				while (it1.hasNext()) {
					State currSt = it1.next();
					if (!aut1FinalCopy.contains(currSt)) {
						aut1.finalSt.add(currSt);
					}
				}
			}
			...
			//System.out.print("Here is the intersection: \n");			
	...
	public Automaton2 determinize (Automaton2 a) {
		...
		Object[] alphStrs = a.sortedAlph.toArray();
			...
			while (it.hasNext()) {
				...
				for (int i = 0; i < numSyms; i++) {
					...
					for (int j = 0; j < a.sortedAlph.size(); j++) {
						...
						if ((i & bitMask) > 0) alph = alph + (String) alphStrs[j] + ":";
						else alph = alph + "!" + (String)alphStrs[j] + ":";
					}
		...
		// fix the NFA transitions to use expanded alphabet
		{
			...
			while (it.hasNext()) {
				Transition currTr = a.transitions.get(it.next());
	...
	public Automaton2 intersect(Automaton2 aut1, Automaton2 aut2)
	{
		...
		LinkedList<State> statesToDo = new LinkedList<State>(); // states for which we need to add transitions
		LinkedList<State> statesDone = new LinkedList<State>(); // list of states that have been added
		LinkedList<State> hashSet = new LinkedList<State>();  // create HashSet to hold start states
		hashSet.add(aut1.startSt); // add the start state from aut1
		hashSet.add(aut2.startSt); // add the start state from aut2
		...
		mapping.put(newState, hashSet); // add the mapping of the start states
		...
		while (!statesToDo.isEmpty())
		{
			State curState = statesToDo.getFirst(); // take out the first state
			LinkedList<State> tempHashSet = mapping.get(curState); // use the mapping to get the states that make up the state in a from aut1 and aut2
			Iterator<State> stateIt = tempHashSet.iterator();
			State s1 = stateIt.next();
			State s2 = stateIt.next();
			...
			while (symbolsIt.hasNext())
			{
				...
				LinkedHashSet<State> destStates1 = trans1.get(curSymbol); //get states the transitions go to on that symbol
				LinkedHashSet<State> destStates2 = trans2.get(curSymbol);
			...
			statesToDo.remove(curState);
			statesDone.add(curState);
		}

/*		Iterator<State> aut1FinalStatesIt = aut1.finalSt.iterator();
		while (aut1FinalStatesIt.hasNext())
		{
			State curFinalState = aut1FinalStatesIt.next();
			Iterator<State> mapKeyIt = a.states.iterator();
			while (mapKeyIt.hasNext())
			{
				State mapKey = mapKeyIt.next();
				LinkedList<State> oldStates = mapping.get(mapKey);
				if (oldStates.contains(curFinalState))
				{
					a.finalSt.add(mapKey);
				}
			}
		}
		Iterator<State> aut2FinalStatesIt = aut2.finalSt.iterator();
		while (aut2FinalStatesIt.hasNext())
		{
			State curFinalState = aut2FinalStatesIt.next();
			Iterator<State> mapKeyIt = a.states.iterator();
			while (mapKeyIt.hasNext())
			{
				State mapKey = mapKeyIt.next();
...
*/
		...
		// a state in a is finalSt if corresponding states are final sates in both aut1 and aut2
		for (State mapKey: a.states) {
			LinkedList<State> oldStates = mapping.get(mapKey);
			State aut1St = oldStates.getFirst();
			State aut2St = oldStates.getLast();
			if (aut1.finalSt.contains(aut1St) && aut2.finalSt.contains(aut2St)) a.finalSt.add(mapKey);
		}
		
		// WITHOUT WINDOWS IMPLEMENTATION -- Buffers of intersection = Buffers of aut2
		{ // copy buffers and actions
			Iterator<Buffer> it = aut2.buffers.iterator();
			while (it.hasNext()) {
				Buffer b = it.next();
				Buffer b1 = new Buffer();
				a.addBuffer(b1);
		...
		{ // copy state-buffer-links
			Iterator<State> it = aut2.stateBufferLink.keySet().iterator();
			while (it.hasNext()) {
				State s = it.next();
				Iterator<Buffer> it1 = aut2.stateBufferLink.get(s).iterator();
				while (it1.hasNext()) {
					Buffer b = it1.next();
					Iterator<State> it2 = mapping.keySet().iterator();
					while (it2.hasNext()) {
						State intSt = it2.next();
						LinkedList<State> setSt = mapping.get(intSt);
						if (setSt.contains(s))
							a.addStateBufferLink(intSt, a.getBuffer(b.num));
					}
				}
			}
		}
		// set a start buffers = aut2 start buffers
		{
			Iterator<Buffer> it = aut2.startBuf.iterator();
			while (it.hasNext()) {
				Buffer b1 = it.next();
				a.startBuf.add(a.getBuffer(b1.num));
			}
		}

		// set aut final buffer = new buffer created
		a.finalBuf = a.getBuffer(aut2.finalBuf.num);

		Automaton2.stateNum++;
		Automaton2.bufferNum++;

		{
			// System.out.println ("Done some intersection");
			// System.out.println ("Mapping is");
			Set<State> keySet = mapping.keySet();
			Iterator<State> it2 = keySet.iterator();
			while (it2.hasNext()) {
				State detSt = it2.next();
				String s = "";
				LinkedList<State> ndSt = mapping.get(detSt);
				Iterator<State> it3 = ndSt.iterator();
				while (it3.hasNext()) {
					s = s + it3.next().num + "::";
				}
	...
	public void display() {
		...
		{
			System.out.println("Start State : " + "NULL");
		}
		...
		{
			System.out.println("Final Buffer : " + "NULL");
		}
